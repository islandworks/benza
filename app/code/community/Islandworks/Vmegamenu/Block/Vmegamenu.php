<?php
class Islandworks_Vmegamenu_Block_Vmegamenu extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false, $class='')
    {
		...
		if($catdetail->getData('mtmenu_cat_icon')) {
			 $html[] = '<i class="fa '.$catdetail->getData('mtmenu_cat_icon').'"></i><span>' . $name . '</span>';
		} else {
			$html[] = '<span>' . $name . '</span>';
		}
        $html[] = '</a>';
	    ...
	    $html[] = '<div class="arrow-left-menu" id="arrow-left-menu' . $id . '">';
            $html[] = '</div>';
    ...
    public function drawColumns($children, $id)
    {
        $html = '';
        // --- explode by columns ---
        $columns = (int)Mage::getStoreConfig('vmegamenu/columns/count');
    ...
    protected function getActiveChildren($parent, $level)
    {
        ...
        // --- check level ---
        $maxLevel = (int)Mage::getStoreConfig('vmegamenu/general/max_level');
        if ($maxLevel > 0)
        {
   ...
   function partition_element(Array $list, $p) {
    ...
    public function drawMenuItem($children, $level = 1, $columChunk)
    {
        ...
        $catsid = Mage::getStoreConfig('vmegamenu/general/catsid');
        if($catsid){    
...
//        print_r($arr_catsid);die();
    ...
    public function drawCustomMenuBlock($blockId)
    {
        ...
        $html = array();
        // --- Static Block ---
        //$blockId = sprintf('pt_vmegamenu_%d', $id); // --- static block key
